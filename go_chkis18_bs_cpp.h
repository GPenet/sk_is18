...
//#define DEBUG3 76
//#define DEBUG4 7727
//#define DEBUG5 12859251
//#define DEBUG3 128
//#define DEBUG4 12870
//#define DEBUG5 21205939
//#define DEBUG4 136600
//#define DEBUG5 1921951
...
#endif
#ifdef DEBUG4	
	if (p_cpt2g[4] == DEBUG4)chunkh.Debug4Clean();
#endif
	while (pw < myend) {
		p_cpt2g[5]++;
		myb12 = *pw++;
		...
		//int locdiag = 0;
		if (p_cpt2g[5] == DEBUG5) {
			cout << Char54out(myb12) << " p_cpt2g[5] debug " << p_cpt2g[5] << endl;
			locdiag = 1;
		}
...
#ifdef DEBUG5 
			if (locdiag)
				cout << "  back  call init go b3 nclues=lim "   << endl;
...
#ifdef DEBUG5 
	if (locdiag) 
		cout << "  back init go b3  " << p_cpt2g[5] << endl;
...
void GCHK::CheckValidBelow(uint64_t bf, uint64_t ac) {
	...
	//int locdiag = 0;
	if (p_cpt2g[5] == DEBUG5) {
		cout << Char54out(myb12) << " check below p_cpt2g[5] debug " << p_cpt2g[5] << endl;
		locdiag = 1;
	}
...
#ifdef HAVEKNOWN
	if (locdiag) {
		cout << "  valid enter the add process" << endl;
	}
#endif	
	p_cpt2g[17]++;
...
void GCHK::ExpandAddB1B2(uint64_t bf) {// add up to n cells
	...
	if (lim > 2) {// for debugging catch more than one add
		cout << " more than one add lim=" << lim << " p_cpt2g[17]=" << p_cpt2g[17] << endl;
		cout << Char54out(bf) << " bf mynclues=" << mynclues << " ntadd=" << ntadd << endl;
	}
#ifdef DEBUG5 
	if (p_cpt2g[5] == DEBUG5) {
		cout << "  add lim=" << lim << " p_cpt2g[17]=" << p_cpt2g[17] << endl;
		locdiag = 1;
	}
...
#ifdef HAVEKNOWN
			if (locdiag==3) 				cout << "checkk bug5 miss2" << endl;

#endif	
			if (vbx.ntof < 3 || vbx.GetAnd()) isdirect = 1;
			else {
#ifdef HAVEKNOWN
				if (locdiag == 3) 				cout << "check bug5continue miss2 " << endl;
...
void  GCHK::BuildExpandB3Vect( uint32_t cl0bf, uint32_t active0,
...
#ifdef DEBUG5
	if (locdiag) 
		cout << " back  expand b3direct.nt=" << b3direct.nt << endl;
	...
	b3direct.nt = 0;
#ifdef DEBUG5
	if (locdiag) 		cout << " back expand" << endl;
#endif
}
